...
export class IndirectUrlResolver extends FsUrlResolver {
  ...
  constructor(
    ...
    for (const [u, fsPath] of indirectionMap) {
      ...
      const fsUrl = rootResolver.resolve(this.brandAsRelative(fsPath));
      if (fsUrl === undefined) {
  ...
  resolve(url: FileRelativeUrl|PackageRelativeUrl, baseUrl?: ResolvedUrl):
      ResolvedUrl|undefined {
    // If we're just resolving user input, do the basic thing.
    if (baseUrl === undefined) {
      return super.resolve(url);
    }
    const runtimeBaseUrl = this.resolvedUrlToRuntimeUrl.get(baseUrl);
    if (runtimeBaseUrl === undefined) {
      return super.resolve(url, baseUrl);
    }
  ...
  private runtimeResolve(
    ...
    return this.simpleUrlResolve(url, this.brandAsResolved(runtimeBaseUrl)) as
        any as RuntimeUrl;
  }

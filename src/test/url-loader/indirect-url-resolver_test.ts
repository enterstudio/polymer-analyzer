...
import {fileRelativeUrl, rootedFileUrl} from '../test-utils';
...
suite('IndirectUrlResolver', function() {
  ...
  test('the full use case', async () => {
    ...
    overlayLoader.urlContentsMap.set(
        fsUrlResolver.resolve(fileRelativeUrl`sub/package/foo/foo.html`)!, `
      <link rel="import" href="../bar/bar.html">
      <link rel="stylesheet" href="foo.css">
    `);
    overlayLoader.urlContentsMap.set(
        fsUrlResolver.resolve(fileRelativeUrl`sub/package/foo/foo.css`)!, ``);
    overlayLoader.urlContentsMap.set(
        fsUrlResolver.resolve(fileRelativeUrl`different/x/y/bar.html`)!, `
      <link rel="stylesheet" href="./bar.css">
    `);
    overlayLoader.urlContentsMap.set(
        fsUrlResolver.resolve(fileRelativeUrl`different/x/y/bar.css`)!, ``);
    const analyzer =
  ...
  test('resolve', async () => {
    ...
    assert.deepEqual(
        indirectResolver.resolve(fileRelativeUrl`foo.html`),
        rootedFileUrl`root/sub/package/foo.html`);

    // Full URLs are left alone
    assert.deepEqual(
        indirectResolver.resolve(
            rootedFileUrl`root/sub/package/bar.html` as any as FileRelativeUrl),
